...
export const ProductosTab: React.FC = () => {
  ...
  const cargarDatos = useCallback(async () => {
    try {
      ...
      // Si el usuario es cliente, filtrar solo sus productos
      if (user?.rol === 'cliente' && cliente) {
        const productosDelCliente = productosData.filter(producto => producto.clienteId === cliente.id);
        setProductos(productosDelCliente);
      } else {
  ...
  try {
    ...
    if (user?.rol === 'cliente' && cliente) {
      ...
      // Para taller, validar que se seleccionó un cliente
      if (!data.clienteId) {
      ...
      datosProducto = {
        ...data,
        clienteId: Number(data.clienteId) // CONVERTIR A NÚMERO
      };
    }
