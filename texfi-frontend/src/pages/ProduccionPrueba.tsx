...
export const ProduccionPage: React.FC = () => {
  ...
  const { register, handleSubmit, reset, watch, formState: { errors } } = useForm<ProduccionFormData>({
    defaultValues: {
  ...
  const onSubmit = async (data: ProduccionFormData) => {
    ...
    setCargando(true);
    try {
      // Crear la producción base con fecha actual (now)
      const produccionData: Omit<Produccion, 'id'> = {
        fecha: new Date(), // Usar fecha actual del servidor
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Input
              ...
              error={errors.tiempoPlanificado?.message}
            />
